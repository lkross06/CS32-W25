...
//        swap(s[0], s[1]);
    for (int i = 1; i < s.size(); i++){
        Store nextStore = s.at(i);
        int loc = i;
        while (loc > 0 && comp(nextStore, s.at(loc-1))){
            swap(s.at(loc), s.at(loc-1));
            loc--;
        }
        swap(nextStore, s.at(loc));
    }
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    vector<Store*> ptStores;
    for (auto it = auxStores.begin(); it != auxStores.end(); it++) ptStores.push_back(&(*it));

      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with the comp parameter as the ordering relationship.
    sort(ptStores.begin(), ptStores.end(), comp);
