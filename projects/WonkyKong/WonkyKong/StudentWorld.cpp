...
: GameWorld(assetPath), _player(nullptr), _burps(START_BURPS), _hasWon(false){
    //find the "level__.txt" file with the largest number. this will be the final
    //level in the game. if any level files before/including that are invalid,
    //the player can't win
    Level level(assetPath);
    for (int i = 0; i < MAX_LEVEL_NUM; i++){
        string filename = getLevelFilename(i);
        Level::LoadResult res = level.loadLevel(filename);
...
int StudentWorld::init()
{
    ...
    _burps = START_BURPS; //reset burps
    _hasWon = false;
    
    Level level(assetPath());
    
    //load level details
    string l_filename = getLevelFilename(getLevel());
            ...
            for (int col = 0; col < VIEW_WIDTH; col++){
                ...
                switch (me) {
                    case Level::floor:
                        ...
                        break;
                    case Level::ladder:
                        _actors.push_back(new Ladder(this, col, row));
                        break;
                    case Level::left_kong:
                        _actors.push_back(new Kong(this, col, row, true));
                        break;
                    case Level::right_kong:
                        _actors.push_back(new Kong(this, col, row, false));
                        break;
                    case Level::fireball:
                        _actors.push_back(new Fireball(this, col, row));
                        break;
                    case Level::koopa:
                        _actors.push_back(new Koopa(this, col, row));
                        break;
                    case Level::bonfire:
                        _actors.push_back(new Bonfire(this, col, row));
                        break;
                    case Level::extra_life:
                        _actors.push_back(new ExtraLifeGoodie(this, col, row));
                        break;
                    case Level::garlic:
                        _actors.push_back(new GarlicGoodie(this, col, row));
                        break;
                    case Level::player:
...
int StudentWorld::move()
{
    ...
    //make all dynamic actors do something (we know that static actors do NOTHING)
    //not using an iterator in case any actors get added to the vector
    updateGameText();
    ...
    //this is checked after kong's doSomething runs
    if (_hasWon){
        if (getLevel() > MAX_LEVEL_NUM || getLevel() == _lastLevelNum){
...
void StudentWorld::spawn(Actor* a){
    _actors.push_back(a); //any new actors spawned in must be dynamic
}

void StudentWorld::attackAllActorsAt(int x, int y){
    //in some cases we may need to attack multiple dynamic actors at a particular position
    vector<Actor*>::iterator it = _actors.begin();
    while (it != _actors.end()){
        if ((*it)->isAt(x, y)) (*it)->getAttacked();
        it++;
    }
}

void StudentWorld::burnAllBurnableActorsAt(int x, int y){
    //in some cases we may need to burn multiple burnable actors at a particular position
    vector<Actor*>::iterator it = _actors.begin();
    while (it != _actors.end()){
        if ((*it)->isAt(x, y) && (*it)->isBurnable()) (*it)->getBurned();
        it++;
    }
    //also check player
    if (_player->isAt(x, y) && _player->isBurnable()) _player->getBurned();
}

void StudentWorld::freezePlayer(){ _player->getFrozen(); }
void StudentWorld::attackPlayer(){ _player->getAttacked(); }
...
bool StudentWorld::climbableAt(int x, int y){
    vector<Actor*>::iterator it = _actors.begin();
    while (it != _actors.end()){
        if ((*it)->isAt(x, y) && (*it)->isClimbable()) return true;
        it++;
    }
    return false;
}

int StudentWorld::getPlayerX() const { return _player->getX(); }
int StudentWorld::getPlayerY() const { return _player->getY(); }

string StudentWorld::getLevelFilename(int level_num) const{
    ostringstream level_filename;
    level_filename << "level";
    level_filename.fill('0');
    level_filename << setw(2) << level_num << ".txt";
    return level_filename.str();
}

void StudentWorld::updateGameText(){
    ostringstream oss; //use stringstreams so i can add trailing zeroes
